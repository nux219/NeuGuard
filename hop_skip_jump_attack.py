...
def hop_skip_jump_attack(
    ...
    def hsja(sample, target_label, target_image):
        if target_label is None:
            _, original_label = torch.max(model_fn(sample)[0], 1)
...
def initialize(decision_function, sample, shape, clip_min, clip_max):
    ...
    while True:
        ...
        if success:
            ...
            num_evals += 1
        if num_evals> 5e5:
            print("Initialization too many times! Try to use a misclassified image as `target_image`")
            break
        # num_evals += 1
        # message = (
        #     "Initialization failed! Try to use a misclassified image as `target_image`"
        # )
        # assert num_evals < 5e4, message
